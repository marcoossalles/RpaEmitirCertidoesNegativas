...
class BaixarCertidaoViaApi:
    ...
    def baixa_certidao_api(self, url, cnpj, nome_empresa, tipo, extensao, negativa):
        try:
            self.logging.info(f"Baixando certidão da empresa {nome_empresa}")
            ...
            if response.status_code == 200:
                self.logging.info(f"Certidão baixada com sucesso.")
                
                # Salva o conteúdo como arquivo PDF temporário no diretório de download configurado
                self.logging.info(f"Salvando certidão {self.download_dir}.")
                ...
                self.logging.info(f"PDF renomeado: {cnpj}_certidao.{extensao} -> {nome_empresa}.{extensao}")
                ...
                self.logging.error(f"Erro ao baixar arquivo PDF")
                return "PENDENTE"
